...
export const updateUpgradeItem = async (req, res) => {
    try {
        const item = await UpgradeItem.findByIdAndUpdate(
            req.params.id,
            req.body,
    ...
    try {
        ...
        if (purchasedAs === 'tool') {
            ...
            await logAudit({
                ...
                entityType: 'UpgradeItem',
                entityId: item._id,
                details: { name: item.name, purchasedAs: 'tool', newToolId: tool._id }
            }, req);

            res.json({ item: await item.populate('purchasedToolRef', 'name category'), tool });

        } else if (purchasedAs === 'subtool') {
            const tool = await Tool.findById(toolId);
            if (!tool) return res.status(404).json({ message: 'Target tool not found' });
            ...
            await logAudit({
                ...
                entityType: 'UpgradeItem',
                entityId: item._id,
                details: { name: item.name, purchasedAs: 'subtool', toolId }
            }, req);

            res.json({ item: await item.populate('purchasedToolRef', 'name category'), tool });

        } else {
            res.status(400).json({ message: 'purchasedAs must be "tool" or "subtool"' });
        }
    } catch (error) {
        res.status(400).json({ message: error.message });
    }
...
export const getUpgradeFund = async (req, res) => {
    try {
        ...
        const toolPayoffRevenue = tools.reduce((sum, t) => sum + (t.revenueCounter || 0), 0);
        const totalToolCost = tools.reduce((sum, t) => sum + (t.purchasePrice || 0), 0);
        res.json({
            investmentFund: metrics.upgradeInvestmentFund || 0,
            investmentHistory: (metrics.upgradeInvestmentHistory || []).slice(-30).reverse(),
            toolPayoffRevenue,
            totalToolCost,
        });
    } catch (error) {
...
export const addFundDeposit = async (req, res) => {
    try {
        ...
        metrics.upgradeInvestmentFund = (metrics.upgradeInvestmentFund || 0) + Number(amount);
        if (!metrics.upgradeInvestmentHistory) metrics.upgradeInvestmentHistory = [];
        ...
        await logAudit({
            ...
            details: { amount: Number(amount), description, newBalance: metrics.upgradeInvestmentFund }
        }, req);
...
export const subtractFund = async (req, res) => {
    try {
        ...
        const metrics = await FinancialMetrics.getInstance();
        if (!metrics.upgradeInvestmentHistory) metrics.upgradeInvestmentHistory = [];
        metrics.upgradeInvestmentFund = Math.max(0, (metrics.upgradeInvestmentFund || 0) - Number(amount));
        metrics.upgradeInvestmentHistory.push({
            amount: -Number(amount),

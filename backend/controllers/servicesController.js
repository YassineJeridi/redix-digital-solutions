    ...
    try {
        ...
        if (search) {
            ...
            projects = projects.filter(project => 
                ...
                (project.client && project.client.businessName && project.client.businessName.toLowerCase().includes(searchLower))
            );
        }
...
export const updateService = async (req, res) => {
    try {
        const oldProject = await Service.findById(req.params.id).populate('teamMembers');
...
export const updateServiceStatus = async (req, res) => {
    try {
        ...
        const project = await Service.findById(req.params.id).populate('teamMembers');
...
export const getServiceStats = async (req, res) => {
    try {
        ...
        const activeProjects = await Service.countDocuments({ ...query, projectStatus: 'In Progress' });
        const completedProjects = await Service.countDocuments({ ...query, projectStatus: 'Completed' });
        const pendingPayments = await Service.countDocuments({ ...query, paymentStatus: 'Pending' });

...
const projectSchema = new mongoose.Schema(
  {
    ...
    // Ads fees deducted before revenue distribution
    adsFees: {
      enabled: { type: Boolean, default: false },
      amountUSD: { type: Number, default: 0, min: 0 },
      usdToTND: { type: Number, default: 3.5, min: 0 },
      amountTND: { type: Number, default: 0, min: 0 },
    },
  },
  { timestamps: true },
);
...
projectSchema.pre("save", function (next) {
  ...
  const revTotal =
    this.revenueDistribution.toolsAndCharges +
  ...
  if (this.toolsUsage && this.toolsUsage.length > 0) {
    const toolsTotal = this.toolsUsage.reduce(
      (sum, tool) => sum + tool.percentage,
      0,
    );
  ...
  if (this.teamMemberShares && this.teamMemberShares.length > 0) {
    const sharesTotal = this.teamMemberShares.reduce(

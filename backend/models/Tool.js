...
// Auto-compute purchasePrice = basePrice + sum of subTools (runs before validation)
toolSchema.pre("validate", function (next) {
  const subToolsTotal = (this.subTools || []).reduce(
    (sum, st) => sum + (st.purchasePrice || 0) * (st.quantity || 1),
    0,
  );
...
toolSchema.methods.calculatePayoff = function () {
  if (this.purchasePrice === 0) {
    ...
    this.payoffPercentage = Math.min(
      (this.revenueCounter / this.purchasePrice) * 100,
      100,
    );
  }

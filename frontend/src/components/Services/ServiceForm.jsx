...
import { MdClose, MdSave, MdAdd, MdEdit, MdBusiness, MdAttachMoney, MdGroup, MdBuildCircle, MdPieChart } from 'react-icons/md';
    ...
    useEffect(() => {
        ...
        if (editData) {
            setFormData({
                ...
                serviceStatus: editData.projectStatus || editData.serviceStatus || 'Not Started',
            ...
            <div className={styles.modal}>
                <div className={styles.header}>
                    ...
                    <div className={styles.headerText}>
                        ...
                        <p>{editData ? `Editing: ${editData.projectName || editData.serviceName}` : 'Fill in the details to create a new service project'}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                    ...
                    <section className={styles.section}>
                        <h3 className={styles.sectionTitle}><MdBusiness /> Basic Information</h3>
                    ...
                    <section className={styles.section}>
                        <h3 className={styles.sectionTitle}><MdPieChart /> Revenue Distribution (must total 100%)</h3>
                    ...
                    {showToolsUsage && (
                        <section className={styles.section}>
                            <h3 className={styles.sectionTitle}><MdBuildCircle /> Tools Usage (auto-calculated by purchase price)</h3>
                            {errors.toolsUsage && (
                            ...
                            {formData.toolsUsage.length > 0 && (
                                <div className={styles.infoBox}>
                                    ...
                                    {formData.toolsUsage.reduce((sum, tu) => {
                                        const tool = tools.find(t => t._id === tu.tool);
                                        return sum + (tool?.purchasePrice || 0);
                                    }, 0).toLocaleString()} TND
                                </div>
                            )}
                                        ...
                                        <div key={tool._id} className={`${styles.toolItem} ${usage ? styles.toolItemActive : ''}`}>
                                            <label className={styles.toolLabel}>
                                                ...
                                                <div className={styles.toolNamePrice}>
                                                    <span>{tool.name}</span>
                                                    <span className={styles.toolPrice}>{(tool.purchasePrice || 0).toLocaleString()} TND</span>
                                                </div>
                                            </label>
                                            {usage && (
                    ...
                    <section className={styles.section}>
                        <h3 className={styles.sectionTitle}>
                            <MdGroup /> Team Assignment &amp; Share Distribution (must total 100%)
                        </h3>
                    ...
                    <div className={styles.footer}>
                        ...
                        <button type="submit" className={styles.submitBtn} disabled={loading}>
                            {loading ? <span className={styles.spinner} /> : (editData ? <MdSave /> : <MdAdd />)}
                            {loading ? 'Saving...' : (editData ? 'Update Service' : 'Create Service')}
                        </button>
                    </div>
                </form>
            </div>

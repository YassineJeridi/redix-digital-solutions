...
const ToolsList = () => {
  ...
  const [deleteConfirm, setDeleteConfirm] = useState(null); // { tool } | null
  ...
  const confirmDelete = async () => {
    ...
    try {
      await ToolsService.deleteTool(deleteConfirm.tool._id);
      fetchTools();
    } catch (error) {
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.summaryRow}>
          ...
          <div className={styles.stat}>
            ...
            <span className={styles.statLabel}>Total Tools Price</span>
          </div>
          <div className={styles.stat}>
            <span className={styles.statNum}>
          ...
          <div className={styles.stat}>
            <span className={styles.statNum}>
              {Math.max(0, totalReserved).toLocaleString()} TND
            </span>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {filteredTools.map((tool) => (
                    <tr key={tool._id} className={styles.tableRow}>
                      <td className={styles.toolName}>{tool.name}</td>
                      <td>
                        <span
                      ...
                      <td>{tool.purchasePrice?.toFixed(2)} TND</td>
                      <td>{tool.revenueCounter?.toFixed(2)} TND</td>
                      <td>
                        <div className={styles.payoffCell}>
                          <div className={styles.miniBar}>
                            <div
                              className={styles.miniBarFill}
                              style={{
                                width: `${Math.min(tool.payoffPercentage || 0, 100)}%`,
                              }}
                            />
                          </div>
                          <span>{tool.payoffPercentage?.toFixed(0)}%</span>
                        </div>
                      </td>
                  ...
                  <div className={styles.cardBody}>
                    ...
                    <div className={styles.infoRow}>
                      ...
                      <span className={styles.value}>
                        {tool.purchasePrice?.toFixed(2)} TND
                      </span>
                    </div>
                    ...
                    <div className={styles.progressSection}>
                      <div className={styles.progressHeader}>
                        ...
                        <span className={styles.percentage}>
                          {tool.payoffPercentage?.toFixed(0)}%
                        </span>
                      </div>
                        ...
                        <div className={styles.subToolsList}>
                          {tool.subTools.map((subTool, idx) => (
                            <div key={idx} className={styles.subToolItem}>
                              <span className={styles.subToolName}>
                                {subTool.name}
                              </span>
                              <span className={styles.subToolPrice}>
                                {subTool.purchasePrice} TND{" "}
                                {subTool.quantity > 1
                                  ? `× ${subTool.quantity}`
                                  : ""}
                              </span>
      ...
      {deleteConfirm && (
        ...
        >
          ...
          >
            <h3 className={styles.deleteModalTitle}>Delete Tool</h3>
            <p className={styles.deleteModalText}>
              This action cannot be undone. Type{" "}
              <strong>{deleteConfirm.tool.name}</strong> to confirm deletion.
            </p>
            <input
            ...
            <div className={styles.deleteModalActions}>
              ...
              <button
                ...
                disabled={deleteInput !== deleteConfirm.tool.name}

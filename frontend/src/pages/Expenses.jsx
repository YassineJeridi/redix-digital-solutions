...
const Expenses = () => {
  ...
  const loadSummary = async () => {
    try {
      const summaryData =
        await ExpensesService.getFinancialSummary(chartPeriod);
  ...
  const handleDepositSubmit = async (e) => {
    ...
    try {
      await ExpensesService.addManualDeposit(
        amount,
    ...
    <div className={styles.container}>
      ...
      <div className={styles.summaryGrid}>
        <div
          ...
          style={{ borderColor: summary.balance >= 0 ? "#10b981" : "#ef4444" }}
        >
          <div
          ...
          >
            <MdAccountBalance
              style={{ color: summary.balance >= 0 ? "#10b981" : "#ef4444" }}
            />
          </div>
      ...
      {/* Period filter shared between both charts */}
      ...
      <div className={styles.chartsRow}>
        {/* Redix Caisse — Area chart (running balance) */}
        <div className={styles.chartCard}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            ...
            >
              ...
              />
              <Tooltip
                formatter={(value) => [`${value.toFixed(2)} TND`, "Balance"]}
                contentStyle={{
        ...
        {/* Expenses — Bar chart (per-period amounts) */}
        <div className={styles.chartCard}>
          <div className={styles.chartHeader}>
            <div className={styles.chartTitleGroup}>
              <h3>Expenses</h3>
          ...
          <ResponsiveContainer width="100%" height={280}>
            ...
            >
              ...
              />
              <Tooltip
                formatter={(value) => [`${value.toFixed(2)} TND`, "Expenses"]}
                contentStyle={{
                  background: "var(--card-bg)",
                  border: "1px solid var(--border-color)",
                  borderRadius: "10px",
              ...
              <Bar
                ...
                maxBarSize={36}
              >
                {(summary.chartData || []).map((entry, i) => (
                  <Cell
                    key={i}
                    fill={
                      entry.expenses > 0 ? "#ef4444" : "var(--border-color)"
                    }
                    fillOpacity={entry.expenses > 0 ? 0.85 : 0.4}
                  />
                ))}
              </Bar>
            </BarChart>
      ...
      {showDepositModal && (
        <div className={styles.modalOverlay}>
          <div className={styles.formCard}>
            ...
            {depositError && (
              <div className={styles.errorBox}>{depositError}</div>
            )}
              ...
              <div className={styles.formRow}>
                <div className={styles.formGroup}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setDepositData({ ...depositData, source: e.target.value })
                    }

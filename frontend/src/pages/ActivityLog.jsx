...
const buildSentence = (log) => {
  ...
  const entityName =
    d.projectName || d.businessName || d.name || d.invoiceNumber || "";
  const q = (s) => (s ? ` "${s}"` : "");
  switch (log.action) {
    case "create": {
      let s = `Created ${log.entityType || "record"}${q(entityName)}`;
      return s;
    }
    case "update": {
      let s = `Updated ${log.entityType || "record"}${q(entityName)}`;
      if (d.changes) {
        const fs = Object.keys(d.changes);
        if (fs.length === 1) s += ` — changed ${fs[0]}`;
        else if (fs.length > 1)
          s += ` — changed ${fs.slice(0, -1).join(", ")} and ${fs.slice(-1)[0]}`;
      }
      return s;
    }
    case "delete":
      return `Deleted ${log.entityType || "record"}${q(entityName)}`;
    case "status_change":
    case "payment_status_change": {
      let s = `Changed ${log.action === "payment_status_change" ? "payment status" : "status"}`;
      if (entityName) s += ` of "${entityName}"`;
      const chg = d.changes?.status || d.changes?.paymentStatus;
      if (chg?.from !== undefined) s += ` from "${chg.from}" to "${chg.to}"`;
      return s;
    }
    case "commission":
      return `Recorded commission${d.amount ? ` of ${d.amount} TND` : ""}${q(entityName)}`;
    case "withdrawal":
      return `Processed withdrawal${d.amount ? ` of ${d.amount} TND` : ""}`;
    case "team_assignment":
      return `Assigned team member${entityName ? ` to "${entityName}"` : ""}`;
    case "note_added":
      return `Added note${q(entityName)}${d.description ? `: "${d.description}"` : ""}`;
    case "attachment_added":
      return `Added attachment${q(entityName)}`;
    default:
      return `${am.label} — ${log.entityType || "record"}${q(entityName)}`;
  }
};
...
function ExpandedDetails({ log }) {
  ...
  const diffKeys = Object.keys({ ...ov, ...nv }).filter(
    (k) => JSON.stringify(ov[k]) !== JSON.stringify(nv[k]),
  );
  const detailChanges = d.changes ? Object.entries(d.changes) : [];
  const detailFields = Object.entries(d).filter(
    ([k, v]) =>
      k !== "changes" && v !== null && v !== undefined && typeof v !== "object",
  );
    ...
    >
        ...
        <div className={styles.performerBar}>
          {email && (
            <span className={styles.performerEmail}>
              <MdPerson size={13} /> {email}
            </span>
          )}
          {ip && (
            <span className={styles.ipBadge}>
              <MdComputer size={13} /> {ip}
            </span>
          )}
        </div>
      )}
      ...
      {diffKeys.length > 0 && (
        <div className={styles.diffSection}>
          <div className={styles.diffSectionLabel}>Changed fields</div>
          {diffKeys.map((k) => (
            <div key={k} className={styles.diffRow}>
              <span className={styles.diffKey}>{fmtKey(k)}</span>
              <span className={styles.oldVal}>{String(ov[k] ?? "—")}</span>
              <span className={styles.diffArrow}>→</span>
              <span className={styles.newVal}>{String(nv[k] ?? "—")}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {detailChanges.length > 0 && (
        <div className={styles.diffSection}>
          <div className={styles.diffSectionLabel}>Field changes</div>
          {detailChanges.map(([k, v]) => (
            <div key={k} className={styles.diffRow}>
              <span className={styles.diffKey}>{fmtKey(k)}</span>
              {v?.from !== undefined ? (
                <>
                  <span className={styles.oldVal}>{String(v.from ?? "—")}</span>
                  <span className={styles.diffArrow}>→</span>
                  <span className={styles.newVal}>{String(v.to ?? "—")}</span>
                </>
              ) : (
                <span className={styles.fieldVal}>{String(v)}</span>
              )}
            </div>
          ))}
        </div>
        ...
        <div className={styles.detailsFields}>
          {detailFields.map(([k, v]) => (
            <div key={k} className={styles.fieldRow}>
              <span className={styles.fieldKey}>{fmtKey(k)}</span>
              <span className={styles.fieldVal}>{String(v)}</span>
            </div>
          ))}
        </div>
...
const ActivityLog = () => {
  ...
  const fetchStats = async () => {
    try {
      const d = await AuditServices.getAuditStats();
      setStats(d);
    } catch (e) {
      console.error(e);
    }
      ...
      {loading ? (
        ...
        <div className={styles.timeline}>
          {logs.map((log) => {
            ...
            const email = log.performedBy?.email;
            const isSystem = !log.performedBy && !log.userId;
            ...
            return (
              <div key={log._id} className={styles.logEntry}>
                ...
                <div
                  ...
                  style={{
                    background: am.color,
                    boxShadow: `0 0 0 4px ${am.color}22`,
                  }}
                />

                <div
                  className={`${styles.logCard} ${isExpanded ? styles.logCardExpanded : ""}`}
                  style={{ "--action-color": am.color, cursor: "pointer" }}
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      background: isSystem ? "#6b7280" : avatarClr(name),
                    }}
                  >
                    {isSystem ? (
                      <MdHistory size={16} />
                  ...
                  <div className={styles.logBody}>
                    <div className={styles.logTopRow}>
                      <div className={styles.logUserWrap}>
                        <span className={styles.logUser}>{name}</span>
                        {email && (
                          <span className={styles.logUserEmail}>{email}</span>
                        )}
                      </div>
                      <span
                    ...
                    <p className={styles.logDetails}>{sentence}</p>
                    <time className={styles.logFullDate}>
      ...
      {pagination.pages > 1 && (
        <div className={styles.pagination}>
          ...
          {Array.from({ length: pagination.pages }, (_, i) => i + 1)
            ...
            .map((p, i) =>
              p === "…" ? (
                <span key={`sep-${i}`} className={styles.pageSep}>
                  …
                </span>
              ) : (
                <button
